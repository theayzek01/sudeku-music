...
module.exports = {
  name: 'xox',
  aliases: ['tictactoe', 'xoxoyunu'],
  description: 'Bir arkadaşına XOX (Tic-Tac-Toe) meydan oku!',
  usage: 'xox <@rakip>',
  async run(client, message, args) {
    const challenger = message.author;
    const opponent = message.mentions.users.first() || (args[0] ? await client.users.fetch(args[0].replace(/[^0-9]/g, '')).catch(() => null) : null);

    if (!opponent) {
      return message.reply({
        content: 'kime meydan okuyacaksın kanka? rakibini etiketlemeyi unuttun ya... Örn: `a!xox @rakip`',
      });
    }
